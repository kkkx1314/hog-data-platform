...
.page{max-width:1440px;margin:0 auto;padding:24px 16px 52px;}
.page-wide{max-width:1900px;}
...
.card{background:#fff;border:1px solid #e5e9f0;border-radius:14px;padding:10px;box-shadow:0 1px 4px rgba(15,23,42,.05);}
...
function chartLayout(h, yrange){
  return {
    ...
    margin:{l:32,r:4,t:6,b:28}, height:h,
    ...
    yaxis:{gridcolor:'#eef2f7', zeroline:false, range:yrange, title:null},
...
function render(){
  ...
  PRICE.provinces.forEach(function(prov, i){
    ...
    Plotly.react('chart_'+i, traces, chartLayout(360, paddedYRange(allY)), {responsive:true});
  });
}
...
<div class="page page-wide">{body}</div>
<script>{js}</script>
</body></html>"""
