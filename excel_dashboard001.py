...
def _sync_data_to_github(platform_src: str, transport_src: str) -> dict:
    ...
    try:
        ...
        if platform_dir.exists():
            platform_files = [f for f in platform_dir.glob("*.xlsx") if not f.name.startswith("~$")]
            if platform_files:
                for f in platform_files:
                    dest = repo_data_dir / f.name
                    shutil.copy2(f, dest)
                    files_copied.append(f"📄 {f.name}")
                messages.append(f"✅ 平台数据同步完成：{len(platform_files)} 个文件 → data/")
                messages.append("   " + "、".join(f.name[:25] + ("…" if len(f.name) > 25 else "") for f in platform_files[:6]))
                if len(platform_files) > 6:
                    messages.append(f"   … 及其他 {len(platform_files) - 6} 个文件")
            else:
                messages.append("⚠️ 平台数据目录中没有 .xlsx 文件")
        else:
        ...
        if transport_files:
            ...
            # 复制到平台数据目录和 data/
            dest_platform = platform_dir / latest_file.name if platform_dir.exists() else None
            dest_repo = repo_data_dir / latest_file.name

            if dest_platform:
                shutil.copy2(latest_file, dest_platform)
            shutil.copy2(latest_file, dest_repo)

            date_label = f"{latest_start[:4]}.{latest_start[4:6]}.{latest_start[6:]} – {latest_end[:4]}.{latest_end[4:6]}.{latest_end[6:]}"
            messages.append(f"✅ 调运数据同步完成：{date_label}")
            messages.append(f"   来源：{latest_file.name}")
        ...
        try:
            ...
            if diff_result.stdout.strip():
                commit_result = subprocess.run(
                    ["git", "commit", "-m", "data: 数据更新器同步平台数据"],
                    cwd=str(base_dir),
                    capture_output=True, text=True, timeout=30,
                )
                if commit_result.returncode == 0:
                    messages.append("✅ Git 提交成功")
                else:
                    stderr_short = commit_result.stderr.strip()[-200:] if commit_result.stderr else ""
                    messages.append(f"⚠️ git commit 失败：{stderr_short or '无输出'}")
                ...
                if push_result.returncode == 0:
                    messages.append("✅ Git 推送成功 → GitHub 已更新")
                else:
                    stderr_short = push_result.stderr.strip()[-200:] if push_result.stderr else ""
                    messages.append(f"⚠️ git push 失败：{stderr_short or '无输出'}")
            else:
                messages.append("ℹ️ 数据无变化，跳过 Git 提交")

        except subprocess.TimeoutExpired:
            messages.append("⚠️ Git 操作超时，请检查网络后重试")
        except FileNotFoundError:
            messages.append("⚠️ 未找到 git 命令，请确认 Git 已安装并在 PATH 中")
        except Exception as exc:
            messages.append(f"⚠️ Git 操作异常：{exc}")

        return {"success": True, "messages": messages, "files_copied": files_copied}

    except Exception as exc:
        messages.append(f"❌ 同步过程出错：{exc}")
        return {"success": False, "messages": messages, "files_copied": files_copied}
...
with st.sidebar:
    ...
    with st.expander("📦 数据更新器", expanded=False):
        ...
        if st.button("🚀 执行数据同步", use_container_width=True, key="data_sync_btn",
            ...
            # 同步后自动清除缓存
            st.cache_data.clear()
            st.info("💡 缓存已清除，可切换模块查看最新数据。")
            if st.button("🔄 刷新页面", use_container_width=True, key="post_sync_rerun"):
                st.rerun()
